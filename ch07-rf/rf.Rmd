...
```
```{r}
# select row randomly with replacement and select columnes randomly
...
### By averaging probabilies

```{r}
mean(probs)
```

The average probabliy acress all trees is 0.45 > baseline so the predicted class for `new_data` is "spam". For this new data, we have the same prediction using average probability.
