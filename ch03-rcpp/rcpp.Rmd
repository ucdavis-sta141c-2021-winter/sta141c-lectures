...
Of course, sugar functions could be slow. Instead, we could consider the function `R_IsNA` that checks if a number is NA.
...
NumericVector no_na_fast(NumericVector x) {
  int n = x.size();
  int m = 0;  // the length of the output
  int i, j = 0;
  for (i = 0; i < n; i++) {
    m += !R_IsNA(x[i]);
  }
...
mean(mod$residuals / mtcars$mpg)
```

```{r}
mpeR <- function(mod) {
  if (!inherits(mod, "lm")) {
    stop("Input must be a linear model", call. = FALSE)
  }
...
- C++ Standard Template Library
  ...
  However, it requires some background in C++ in order to disclose its full potential. (I will let you do your own research.)
