...
- weight sharing
- a variety of colorful forms of regularization (e.g., dropout layer)
...
- Backpropagation is a most common algorithm to fit a NN.
    - update the weights by using one observation / a few observation at a time
...
x_train <- mnist$train$x * 1.0
y_train <- mnist$train$y
x_test <- mnist$test$x * 1.0
y_test <- mnist$test$y
...
mnist_model <- keras_model_sequential() %>%
  ...
  layer_lambda(function(x) x / 256) %>%       # scale to 0 and 1
...
PS: due to an [issue](https://github.com/rstudio/reticulate/pull/774) of reticulate, we won't be able to see the progress bar while fitting the model.
...
## Google computer engine and GPU learning
...
- Go to https://console.cloud.google.com/. In the sidebar, go to "API's and Services" -> "Credentials". Click the CREATE CREDENTIALS button and choose "Service account". Following the instruction (you could skip the optional steps) and create a key with JSON type. Save the json file to your computer.
